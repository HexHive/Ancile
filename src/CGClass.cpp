...
llvm::Value *CodeGenFunction::GetVTablePtr(Address This,
  ...
  llvm::MDNode* N = llvm::MDNode::get(C, llvm::MDString::get(C, "vtable"));
  VTable->setMetadata("vtable", N);
...
void CodeGenFunction::EmitVTablePtrCheck(const CXXRecordDecl *RD,
  ...
  EmitSanitizerStatReport(SSK);
   ...
   char temp_md[500];
    ...
    std::string mod_name = Mod->getName().str();
    llvm::Metadata *MD = CGM.CreateMetadataIdentifierForType(QualType(RD->getTypeForDecl(), 0));
    std::string temp_str;
    llvm::raw_string_ostream rso(temp_str);
    MD->print(rso);
    temp_str.erase(std::remove(temp_str.begin(), temp_str.end(), '"'), temp_str.end());
    temp_str.erase(std::remove(temp_str.begin(), temp_str.end(), '!'), temp_str.end());
   std::sprintf(temp_md,"%s:%s",mod_name.c_str(),temp_str.c_str());

    std::string combined_metadata = std::string(temp_md);
    llvm::Metadata *NewMeta = llvm::MDString::get(getLLVMContext(),combined_metadata);

    llvm::Value *TypeId = llvm::MetadataAsValue::get(getLLVMContext(), NewMeta);
    llvm::Value *CastedVTable = Builder.CreateBitCast(VTable,Int8PtrTy);
  ...
  auto CrossDsoTypeId = CGM.CreateCrossDsoCfiTypeId(NewMeta);
