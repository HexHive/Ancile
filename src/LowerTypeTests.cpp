...
void LowerTypeTestsModule::buildBitSetsFromFunctionsNative(
  ...
  for (unsigned I = 0; I != Functions.size(); ++I) {
    ...
    Function *F = dyn_cast<Function>(Functions[I]->getGlobal());
    if (F) {
...
bool LowerTypeTestsModule::lower() {
  ...
  struct cl_inst cl_temp;
  std::vector<cl_inst> cl_set;
  char *path_csv = "/path/to/file.txt";
  std::string stemp;
  std::ifstream infile(path_csv, std::ios_base::in);

  while (getline(infile, stemp, '\n')) {
    std::istringstream ss(stemp);
    std::string token;
    std::vector<std::string> members;
    while (getline(ss, token, ':'))
      members.push_back(token);

    cl_temp.fl_name = members[0];
    cl_temp.line_no = stoi(members[1]);
    cl_temp.col_no = stoi(members[2]);
    cl_temp.metadata = members[3];

    cl_set.push_back(cl_temp);
  }
  char *path_types = "/path/to/file.txt";

  std::map<std::string, std::set<std::string>> types_m;
  std::string temp;
  std::ifstream type_file(path_types, std::ios_base::in);

  while (getline(type_file, temp, '\n')) {
    ...
    std::string func_name = members[0];
    std::istringstream types(members[1]);
    std::set<std::string> tset;
    std::string temp_type;
    while (getline(types, temp_type, ',')) {
      tset.insert(temp_type);
    }

    types_m.insert(
        std::pair<std::string, std::set<std::string>>(func_name, tset));
  }

  LLVMContext &context = M.getContext();
  uint32_t po = 0;
  auto OffsetConst =
      ConstantAsMetadata::get(ConstantInt::get(Type::getInt32Ty(context), po));
  ...
  for (GlobalObject &GO : M.global_objects()) {
    if (isa<GlobalVariable>(GO) && GO.isDeclarationForLinker())
      ...
      if (types_m.find(GO.getName()) != types_m.end()) {

        std::set<std::string>::iterator it;
        for (it = types_m.find(GO.getName())->second.begin();
             it != types_m.find(GO.getName())->second.end(); it++) {
          auto X =
              MDNode::get(context, {OffsetConst, MDString::get(context, *it)});
          Types.push_back(X);
        }
        if (Types.empty())
          continue;
        ...
        if (isa<Function>(GO) && ExportedFunctions.count(GO.getName())) {
          IsDefinition |=
              ExportedFunctions[GO.getName()].Linkage == CFL_Definition;
          IsExported = true;
        }
        auto *GTM = GlobalTypeMember::create(Alloc, &GO, IsDefinition,
                                             IsExported, Types);
  ...
  if (TypeTestFunc) {
   ...
   std::string str;
      ...
      CI->getArgOperand(1)->print(rso);
      

       std::istringstream sstmp(str);
     std::string tkn;
    std::vector<std::string> member;
    while (getline(sstmp, tkn, ':'))
      member.push_back(tkn);

    std::string org_meta = member[1];
    std::string fle_name = member[0];


      int flag_csite = 0;
      if (DebugLoc Loc = CI->getDebugLoc()) {
        ...
        for (auto && s : cl_set) {
          ...
          str.erase(std::remove(str.begin(), str.end(), '!'), str.end());
          if (line == s.line_no && col == s.col_no ){// && str == s.file_name ) {
            flag_csite = 1;
            md_data = s.metadata;
      ...
      if (flag_csite == 1) {
      llvm::Metadata *N = MDString::get(context, md_data);
      AddTypeIdUse(N).CallSites.push_back(CI);
    } else { 

       llvm::Metadata *N = MDString::get(context, org_meta);
  ...
  for (const auto &S : Sets) {
    ...
    for (std::vector<Metadata *>::const_iterator i = TypeIds.begin();
         i != TypeIds.end(); ++i) {      
      (*i)->dump();
