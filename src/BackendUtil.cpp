...
/*Edit for Ancile */
static void addAncilePass(const PassManagerBuilder &Builder,
                                    PassManagerBase &PM) {
   PM.add(createAncilePass());
}

/*Edit for HexCFI */
static void addHexCFIPass(const PassManagerBuilder &Builder,
                                    PassManagerBase &PM) {
   PM.add(createHexCFIPass());
}
...
void EmitAssemblyHelper::CreatePasses(legacy::PassManager &MPM,
  ...
  /* Addded for Ancile */
        if (LangOpts.Sanitize.has(SanitizerKind::Ancile)) {
   PMBuilder.addExtension(PassManagerBuilder::EP_ModuleOptimizerEarly,
                          addAncilePass);
   PMBuilder.addExtension(PassManagerBuilder::EP_EnabledOnOptLevel0,
                          addAncilePass);
  ...
  /* Addded for HexCFI */
        if (LangOpts.Sanitize.has(SanitizerKind::HexCFI)) {
   PMBuilder.addExtension(PassManagerBuilder::EP_ModuleOptimizerEarly,
                          addHexCFIPass);
   PMBuilder.addExtension(PassManagerBuilder::EP_EnabledOnOptLevel0,
                          addHexCFIPass);
